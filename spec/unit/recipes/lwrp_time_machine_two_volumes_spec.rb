...
RSpec.shared_examples 'lwrp_time_machine_two_volumes' do
  ...
  it 'sets the config options in the nodes attributes' do
    ...
    expect(config).to match_array %w(one two)
  end

  it 'creates the path for each volume' do
    expect(chef_run).to create_directory('/srv/tm_one')
    expect(chef_run).to create_directory('/srv/tm_two')
  end
    ...
    supported_platforms.each do |platform, versions|
      versions.each do |version|
        context "on #{platform} v#{version}" do
          ...
          it_behaves_like 'lwrp_time_machine_two_volumes'
        end
      end
    end
  end
